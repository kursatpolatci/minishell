...
/*   Updated: 2024/02/24 13:51:54 by kpolatci         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (type == D_INPUT_R || type == D_OUTPUT_R
		|| type == OUTPUT_R || type == INPUT_R)
		return (1);
	return (0);
}
...
t_execute	*ft_exec_box(t_execute **exec, t_parser *main)
{
	...
	else if (main->type == ARG
		&& (check_prev_type(main->prev->type) == 0) && main->str)
	{
		new_exec = ft_init_exec_box(main->str);
		ft_add_exec_box(exec, new_exec);	
	}
